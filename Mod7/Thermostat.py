...
## Thread lock to prevent concurrent I2C bus collision between threads
i2c_lock = Lock()
...
class TemperatureMachine(StateMachine):
    ...
    def updateLights(self):
        ...
        # Update status lights based on active state and temperature relative to setpoint
        if self.current_state_value == 'heat':
            if temp < self.setPoint:
                redLight.pulse()
            else:
                redLight.on()
        elif self.current_state_value == 'cool':
            if temp > self.setPoint:
                blueLight.pulse()
            else:
                blueLight.on()
        elif self.current_state_value == 'off':
            redLight.off()
            blueLight.off()
    ...
    ##
    ## Cached temperature to handle transient bus glitches
    ##
    lastTemp = 72.0
    ...
    def getFahrenheit(self):
        try:
            with i2c_lock:
                ...
                self.lastTemp = (((9/5) * t) + 32)
                return self.lastTemp
        except Exception as e:
            if(DEBUG):
                print(f"* I2C read transient error: {e}, using cached temp.")
            return getattr(self, 'lastTemp', 72.0)
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                ...
                state_str = str(self.current_state_value).capitalize()
                lcd_line_2 = f"{state_str} Set: {self.setPoint}F"
                altCounter = altCounter + 1
                if(altCounter >= 11):
